...
app.get('/getTeachSubComments/:SubmissionId',async(req,res)=>{
    try {
        ...
        const getTeachSubComments = await pool.query(`SELECT
        *
    FROM
        "Comment"
    WHERE
        "Comment"."submissionId" = $1 AND
        "Comment"."authorType" = 'teacher'`,[SubmissionId])
...
app.get('/getStuSubComments/:StudentId/:SubmissionId',async(req,res)=>{
    try {
        ...
        const getStudentComments = await pool.query(`SELECT
        "Comment".*
    FROM
        "Comment"
    WHERE
        "Comment"."authorId" = $1 AND
        "Comment"."submissionId" = $2`,[StudentId,SubmissionId])
...
app.get('/getStudentDetails/:StudentId',async(req,res)=>{
    try {
        ...
        const getStudentDetails = await pool.query(`SELECT
        *
    FROM
        "Person"
    WHERE
        "Person"."id"  =$1`,[StudentId])
...
app.get('/getAllStudentHomeWorks/:HomeWorkId',async(req,res)=>{
    try {
        ...
        const getStudentsHomeWorks = await pool.query(`SELECT
        "Submission".*
    FROM
        "Submission"
    WHERE
        "Submission"."homeworkId" =$1`,[HomeWorkId])
